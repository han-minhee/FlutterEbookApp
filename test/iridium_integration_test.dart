...
void main() {
  group('Iridium Integration Tests', () {
    ...
    testWidgets('Local Iridium reader creates widget from path', (WidgetTester tester) async {
    ...
    testWidgets('Both implementations have same interface', (WidgetTester tester) async {
      const testPath = '/test/path/book.epub';
    ...
    testWidgets('Factory creates correct reader based on configuration', (WidgetTester tester) async {
    ...
    group('Feature Parity Tests', () {
      testWidgets('Both readers support createEpubReaderFromPath', (WidgetTester tester) async {
        const testPath = '/test/path/book.epub';
        
        final localWidget = localReader.createEpubReaderFromPath(filePath: testPath);
        final externalWidget = externalReader.createEpubReaderFromPath(filePath: testPath);
        
        expect(localWidget, isA<Widget>());
        expect(externalWidget, isA<Widget>());
      });

      testWidgets('Both readers support createEpubReaderFromFile', (WidgetTester tester) async {
        // Mock file object for testing
        final mockFile = MockFile();
        
        final localWidget = localReader.createEpubReaderFromFile(file: mockFile);
        final externalWidget = externalReader.createEpubReaderFromFile(file: mockFile);
        
        expect(localWidget, isA<Widget>());
        expect(externalWidget, isA<Widget>());
      });

      testWidgets('Both readers support createEpubReaderFromUri', (WidgetTester tester) async {
        const testUri = 'https://example.com/book.epub';
        
        final localWidget = localReader.createEpubReaderFromUri(rootHref: testUri);
        final externalWidget = externalReader.createEpubReaderFromUri(rootHref: testUri);
